...
! function(a) {
	a.Editable.DEFAULTS = a.extend(a.Editable.DEFAULTS, {
		fontList: ["黑体", "宋体", "微软雅黑", "Arial,Helvetica", "Courier,Courier New", "Georgia", "Times New Roman,Times", "Trebuchet MS", "Verdana,Geneva"]
	}), a.Editable.prototype.refreshFontFamily = function() {
		var b = a(this.getSelectionElement());
		this.$editor.find('.fr-dropdown > button[data-name="fontFamily"] + ul li').removeClass("active"), this.$editor.find('.fr-dropdown > button[data-name="fontFamily"] + ul li[data-val="' + b.css("font-family") + '"]').addClass("active")
		...
		for(var a = '<ul class="fr-dropdown-menu">', b = 0; b < this.options.fontList.length; b++) {
			var c = this.options.fontList[b],
				d = '<li data-cmd="fontFamily" data-val="' + c + '">';
			d += '<a href="#" data-text="true" title="' + c + '" style="font-family: ' + c + ';">' + c.split(",").join(", ") + "</a></li>", a += d
		}
		return a += "</ul>"
	}
}(jQuery);