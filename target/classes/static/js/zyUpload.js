		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
	            	...
	            	html += '						<input id="fileImage" accept="image/*" type="file" size="30" name="fileselect[]" '+multiple+'>';
	            	html += '					</div>';
	            	html += '				</div>';
					...
					html += '			<input type="button" id="fileSubmit" class="upload_submit_btn" value="确认上传文件"/>';
					html += '		</div>';
	            ...
	            $(self).append(html).css({"width":para.width});
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					if (file.size >=2097152) {
						alert('您这个"'+ file.name +'"文件大小过大,请传小于2M的图片');	
					} else {
						// 在这里需要判断当前所有文件中
						arrFiles.push(file);	
					}
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "/images/zyUpload/fileType/";
			...
			this.addEvent = function(){
				...
				$("#fileImage").bind("onchange", function(e){
					ZYFILE.funGetFiles(e);
				})
			};
