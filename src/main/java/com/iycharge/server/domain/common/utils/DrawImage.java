...
public class DrawImage {
	 ...
	 */
	    ...
	    public static final int HEIGHT = 34;//生成的图片的高度

	    public void drawFunction(HttpServletRequest request, HttpServletResponse
	            response, String createTypeFlag) throws IOException{
...
/*	        String realPath = DrawImage.class.getClassLoader().getResource("").getFile();
	        String fileName = File.separator+"images"+File.separator+"drawImages";
            ...
            if(!file.exists()){
            	...
            	if(child.length>0){
            		for(String ch:child){
            			File filechild = new File(realPath+File.separator+ch);
            			filechild.delete();
            		}
            	}
            }
            String imgpath = realPath+File.separator+random;
            java.io.File img = new java.io.File(imgpath);
	        try {
	        ...
	        //String pimage  = "images"+File.separator+"drawImages"+File.separator+random;
	        //drawbean.setImage(pimage);
	        drawbean.setRandom(random);
	        //return drawbean;
	        //	        //7.将随机数存在session中
	    ...
	    private void setBorder(Graphics g) {
	        // 设置边框颜色
	        g.setColor(Color.getHSBColor(204f,204f,204f));
