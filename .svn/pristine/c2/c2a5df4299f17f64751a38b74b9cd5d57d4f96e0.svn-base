...
! function(a) {
	...
	b.initializers = [], b.count = 0, b.VALID_NODES = ["P", "PRE", "BLOCKQUOTE", "H1", "H2", "H3", "H4", "H5", "H6", "DIV", "LI", "TD", "TH"], b.LANGS = [], b.DEFAULTS = {
		...
		if("string" == typeof d && (b = d), b = b.replace(/<img /gi, '<img data-fr-image-pasted="true" '), b.match(/(class=\"?Mso|style=\"[^\"]*\bmso\-|w:WordDocument)/gi) ? c = this.wordClean(b) : (c = this.clean(b, !1, !0), c = this.removeEmptyTags(c)), this.options.plainPaste && (c = this.plainPasteClean(c)), "" !== c) {
			for(var e = !1, f = a("<div>").append(c).find("*"), g = 0; g < f.length; g++)
				if(a.Editable.VALID_NODES.indexOf(f[g].tagName) >= 0) {
		...
		if(!this.isHTML) {
			...
			if("" === this.text() && d && (0 === d.startOffset || d.startContainer === this.$element.get(0) || !this.inEditor(d.startContainer))) {
				...
				if(g.length >= 1 && g[0] !== this.$element.get(0))
					for(e = 0; e < g.length; e++)
						if(f = g[e], !this.textEmpty(f) || this.browser.msie) return void this.setSelection(f);
				...
				for(g = this.$element.find(b.VALID_NODES.join(",")), e = 0; e < g.length; e++)
					if(f = g[e], !this.textEmpty(f) && 0 === a(f).find(a.Editable.VALID_NODES.join(",")).length) return void this.setSelection(f);
		...
		if(this.browser.msie || this.$element.find(a.Editable.VALID_NODES.join(":empty, ")).remove(), !this.isHTML)
...
function(a) {
	a.Editable.commands = {
		...
		for(var f, g = function(a) {
			...
			var i = a(e[h]);
			if(!this.fakeEmpty(i))
				if(f = "n" == b ? this.options.paragraphy ? a("<p>").html(i.html()) : i.html() + "<br/>" : a("<" + b + ">").html(i.html()), i.get(0) == this.$element.get(0) || g(f)) i.html(f);
				else {
					var j = i.prop("attributes");
					if(f.attr)
						for(var k = 0; k < j.length; k++) "class" !== j[k].name && f.attr(j[k].name, j[k].value);
					var l;
	...
	a.Editable.TAG_ORDER = ["table", "thead", "tbody", "tfoot", "tr", "th", "td", "ul", "ol", "li", "h1", "h2", "h3", "h4", "h5", "h6", "pre", "blockquote", "p", "div", "a", "strong", "em", "strike", "u", "span", "iframe", "object", "param", "video", "source"], a.Editable.SEPARATE = ["th", "td", "li", "h1", "h2", "h3", "h4", "h5", "h6", "pre", "blockquote", "p", "div"], a.Editable.END_AT_SAME_INDEX = ["table", "thead", "tbody", "tfoot", "tr", "th", "td", "a", "span", "iframe", "div", "object", "param"], a.Editable.prototype.isClosingTag = function(a) {
			...
			if(d = b.charAt(o), "<" == d) {
				...
				if(-1 !== p) {
					...
					var q = this.isClosingTag(e),
						...
						t = {};
					if(s != e && (s = s.substring(1, s.length - 1), matches = s.match(/([^:]*):([^:;]*;|$)/gi)))
					...
					if(r = e.match(/([\w\-]*)=("[^<>"]*"|'[^<>']*'|\w+)/gi))
						for(var x = 0; x < r.length; x++) 0 === r[x].indexOf("style=") && r.splice(x, 1);
					if(q) {
						var y = m[g].pop();
		...
		for(b = "", e = 0; e <= k.length; e++) {
			var n = [],
				o = "";
			if(j[e])
				for(j[e] = j[e].sort(this.sortSimpleTags), g = 0; g < j[e].length; g++) a.Editable.SELF_CLOSING_AFTER.indexOf(j[e][g].tag_name) >= 0 ? o += j[e][g].content : b += j[e][g].content;
			if(m[e]) {
				for(var p in m)
					if(p > e)
						for(f = 0; f < m[p].length; f++) {
							var q = m[p][f];
			...
			if(l[e])
				for(l[e] = l[e].sort(this.sortNodes), f = 0; f < l[e].length; f++) d = l[e][f], m[d.end] || (m[d.end] = []), m[d.end].push(d), b += this.openTag(d);
